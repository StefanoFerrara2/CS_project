...
d = 9;          % dimension of uncertain input
...
h = boxplot([mc_sm(:,1), mf_sm(:,1), mc_sm(:,2), mf_sm(:,2),mc_sm(:,3),mf_sm(:,3),mc_sm(:,4),mf_sm(:,4),mc_sm(:,5),mf_sm(:,5),...
    mc_sm(:,6),mf_sm(:,6),mc_sm(:,7),mf_sm(:,7),mc_sm(:,8),mf_sm(:,8),mc_sm(:,9),mf_sm(:,9)],...
    'Colors',[blue; red; blue; red;blue;red;blue;red;blue;red;blue;red;blue;red;blue;red;blue;red],'Whisker',10,...
    'labels',{'MC $s_m^1$','MF $s_m^1$','MC $s_m^2$','MF $s_m^2$','MC $s_m^3$','MF $s_m^3$',...
    'MC $s_m^4$','MF $s_m^4$','MC $s_m^5$','MF $s_m^5$','MC $s_m^6$','MF $s_m^6$',...
    'MC $s_m^7$','MF $s_m^7$','MC $s_m^8$','MF $s_m^8$','MC $s_m^9$','MF $s_m^9$'});
...
title([method,' main sensitivity estimates for steel column model'],'interpreter','latex')
...
if strcmp(method,'Owen') || strcmp(method,'Saltelli')
    ...
    bp = gca;   bp.XAxis.TickLabelInterpreter = 'latex';
    title([method,' total sensitivity estimates for steel column model'],'interpreter','latex')
end
...
title('Variance estimates for steel column model','interpreter','latex')